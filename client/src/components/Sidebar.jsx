...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-2 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer mt-10'>
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
            <div
             ...
             key={chat._id} className='p-2 px-4 rounded-md dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 cursor-pointer flex justify-between group'>
              <div>
              ...
              <img src={assets.bin_icon} className='hidden group-hover:block w-4 cursor-pointer not-dark:invert' onClick={e => toast.promise(deleteChat(e, chat._id),{loading: 'deleting...'})}/>
            </div>
          ))
        }
      </div>
      ...
      <div className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
        ...
        <p className='flex-1 text-sm drk:text-primary truncate'>{user ? user.name : "Login your account"}</p>
